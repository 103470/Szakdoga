...
<div class="body-container my-5">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item">
                    <a href="{{ route('kategoria', ['brandSlug' => $rareBrand->slug, 'typeSlug' => $type->slug, 'vintageSlug' => $vintage->slug, 'modelSlug' => $model->slug]) }}">{{ $model->name }}</a>
                </li>
                <li class="breadcrumb-item">
                    <a href="{{ route('kategoria', ['brandSlug' => $rareBrand->slug, 'typeSlug' => $type->slug, 'vintageSlug' => $vintage->slug, 'modelSlug' => $model->slug, 'categorySlug' => $category->slug]) }}">{{ $category->name }}</a>
                </li>
                @if(isset($subcategory))
                    <li class="breadcrumb-item">
                        <a href="{{ route('alkategoria', ['brandSlug' => $rareBrand->slug, 'typeSlug' => $type->slug, 'vintageSlug' => $vintage->slug, 'modelSlug' => $model->slug, 'categorySlug' => $category->slug, 'subcategorySlug' => $subcategory->slug]) }}">{{ $subcategory->name }}</a>
                    </li>
                @endif
    ...
    <h2 class="mb-4">
        @if(isset($productCategory))
            {{ $subcategory->name ?? $category->name }} – {{ $productCategory->name }} termékkínálatunkból
