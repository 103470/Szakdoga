...
@section('content')
<div class="container">
    <h1>Új ritka évjárat hozzáadása</h1>
    <a href="{{ route('admin.ritkamarkak.evjaratok.index') }}" class="btn btn-secondary mb-3">← Vissza a listához</a>

    <form action="{{ route('admin.ritkamarkak.evjaratok.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-3">
            ...
            <select name="body_type" id="body_type" class="form-control" required>
                ...
                @foreach($bodyTypes as $body)
                    <option value="{{ $body }}" {{ old('body_type') == $body ? 'selected' : '' }}>{{ ucfirst($body) }}</option>
                @endforeach
            </select>
        </div>
