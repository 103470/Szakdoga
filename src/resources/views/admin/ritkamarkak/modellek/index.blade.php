...
@section('content')
<div class="container">
    <h1>Ritka modellek</h1> 
    <a href="{{ route('admin.ritkamarkak.modellek.create') }}" class="btn theme-blue-btn mb-3">+ Új modell</a>
    ...
    <table class="table table-striped w-100">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $model->fuelType ? ucfirst($model->fuelType->name) . ' ' : '' }}{{ $model->name }}</td>
                ...
                <td class="text-end"> 
                    <a href="{{ route('admin.ritkamarkak.modellek.edit', $model->id) }}" class="btn btn-sm btn-warning">Szerkesztés</a>
                    <form action="{{ route('admin.ritkamarkak.modellek.destroy', $model->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
